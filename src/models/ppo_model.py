...
class PPOModel(BaseRLModel):
    ...
    def predict(self, state: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            # Debug state
            print(f"Raw state - min: {torch.min(state_tensor).item()}, max: {torch.max(state_tensor).item()}")
            print(f"State contains NaN: {torch.isnan(state_tensor).any().item()}")
            
            # Apply input normalization
            normalized_state = self.input_bn(state_tensor)
            
            # Debug normalized state
            print(f"Normalized state - min: {torch.min(normalized_state).item()}, max: {torch.max(normalized_state).item()}")
            print(f"Normalized state contains NaN: {torch.isnan(normalized_state).any().item()}")
            ...
            print(f"std min: {torch.min(std).item()}, std max: {torch.max(std).item()}")
            print(f"mu min: {torch.min(mu).item()}, mu max: {torch.max(mu).item()}")
    ...
    def train_step(self, states: np.ndarray, actions: np.ndarray,
              ...
              old_logp: np.ndarray) -> Dict[str, float]:
        """Train the PPO model on a batch of data"""
    ...
    def save(self, path: str) -> None:
        """Save model weights"""
        torch.save(self.actor_mean.state_dict(), f"{path}/ppo_actor_mean")
        torch.save(self.actor_std.state_dict(), f"{path}/ppo_actor_std")
        torch.save(self.critic.state_dict(), f"{path}/ppo_critic")
    
    def load(self, path: str) -> None:
        """Load model weights"""
        self.actor_mean.load_state_dict(torch.load(f"{path}/ppo_actor_mean"))
        self.actor_std.load_state_dict(torch.load(f"{path}/ppo_actor_std"))
        self.critic.load_state_dict(torch.load(f"{path}/ppo_critic"))
